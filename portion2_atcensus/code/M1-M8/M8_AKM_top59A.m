% M8: AKM, worker and industry-cz FEs, time-varying controls
...
disp('AKM estimation - model 8 - top 59A')
...
fsize=fsize(sel); age=age(sel); cz=cz(sel); cz_old=cz_old(sel); naics4d=naics4d(sel);
naics4d_old=naics4d_old(sel); N=length(y); 
...
disp('Saving FEs & PEs by ind-CZ')
out1=[cz_old naics4d_old fe pe];
[ii,jj,kk]=unique(out1(:,1:3), 'rows', 'stable');
out2=[ii,accumarray(kk,1),accumarray(kk,out1(:,4),[],@mean)];
s=['[DATA]/M8_AKM_top59A.txt'];
dlmwrite(s, out2, 'delimiter', '\t', 'precision', 16);
clear out1 out2;

disp('Saving main effects')
out=[id_old cz_old naics4d_old year pe fe];
s=['[DATA]/M8_AKM_top59A_all.txt'];
dlmwrite(s, out, 'delimiter', '\t', 'precision', 16);
clear out;
