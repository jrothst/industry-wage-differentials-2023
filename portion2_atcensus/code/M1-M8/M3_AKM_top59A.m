% M3: OLS, industry FEs, time-varying controls + time-invariant controls
...
disp('AKM estimation - model 3 - top 59A')
...
forborn=data(:,12);
toc
clear data %%%%% UNDO THIS WHEN TESTING FULL DATA %%%%%
...
sex=sex(sel); race=race(sel); hisp=hisp(sel); forborn=forborn(sel); 
...
size(R) %tabulate(year)

% age cubic
age=((age-40)/40); %rescale to avoid big numbers
A=[age, age.^2, age.^3];

% Build time-invariant variables: sex, race, hisp, forborn
SX=sparse(sex);
RC=sparse(1:NT,race',1);
RC(:,1)=[]; %drop first race category
size(RC)
HSP=sparse(hisp);
FB=sparse(forborn);
...
xx=X'*X;
xy=X'*y;
L=ichol(xx,struct('type','ict','droptol',1e-2,'diagcomp',5));
b=pcg(xx,xy,1e-10,1000,L,L');
toc
disp('Done')
clear xx xy L

% clearvars -except data % deletes all but data in workspace
%% ANALYZE RESULTS
xb=X*b;
r=y-xb;
...
out1=[firmid_old fe];
[ii,jj,kk]=unique(out1, 'rows', 'stable');
out2=[ii,accumarray(kk,1)];
s=['[DATA]/M3_AKM_top59A.txt'];
dlmwrite(s, out2, 'delimiter', '\t', 'precision', 16);
clear out1 out2;

disp('Saving xb')
out=[id_old year xb];
s=['[DATA]/M3_AKM_top59A_xb.txt'];
dlmwrite(s, out, 'delimiter', '\t', 'precision', 16);
